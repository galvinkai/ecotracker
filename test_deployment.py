...
def test_backend(backend_url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'transactions' in data and 'chartData' in data:
                print_success(
                    "Successfully retrieved transactions and chart data")
            else:
    ...
    try:
        print_info(
            "Testing POST /transactions endpoint (adding a transaction)...")
        test_transaction = {
        ...
        response = requests.post(
            f"{backend_url}/transactions", json=test_transaction)
        if response.status_code == 200:
            new_transaction = response.json()
            print_success(
                f"Successfully added a new transaction with ID: {new_transaction.get('id', 'unknown')}")
        else:
            print_error(
                f"Failed to add transaction: Status code {response.status_code}")
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print_error(
                f"Failed to get insights: Status code {response.status_code}")
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'prediction' in prediction and 'recommendation' in prediction:
                print_success(
                    f"Successfully received prediction: {prediction['prediction']}")
            else:
                print_error("Prediction response format is not as expected")
        else:
            print_error(
                f"Failed to get prediction: Status code {response.status_code}")
    except Exception as e:
...
def main():
    ...
    if len(sys.argv) > 2:
        ...
        backend_url = input(
            "Enter your backend URL (e.g., https://ecotracker-api.fly.dev): ")
        frontend_url = input(
            "Enter your frontend URL (e.g., https://ecotracker-xyz.vercel.app): ")
