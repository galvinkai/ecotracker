...
def generate_qr_code(url, output_path, title=None, logo_path=None, include_title=True, color="#28a745"):
    ...
    qr_img = qr.make_image(fill_color=color_tuple, back_color="white").convert("RGBA")
    ...
    if title and include_title:
        ...
        full_img = Image.new("RGB", (qr_img.size[0], qr_img.size[1] + padding), "white")
...
def qr_to_html(url, title=None, logo_path=None, color="#28a745"):
    ...
    generate_qr_code(url, img_buffer, title, logo_path, include_title=False, color=color)
    img_buffer.seek(0)
...
def main():
    parser = argparse.ArgumentParser(description='Generate QR code for EcoTracker application')
    ...
    parser.add_argument('--logo', help='Path to logo file to overlay on QR code')
    ...
    generate_qr_code(args.url, args.output, args.title, args.logo, color=args.color)
